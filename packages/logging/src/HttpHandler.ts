...
interface HttpHandlerOptions<AllowBatchRequests extends boolean>
  ...
  /**
   * Options to pass to each `axios` call
   */
...
type HttpHandlerUrlGenerator<AllowBatchRequests extends boolean> = Created<
  ...
  AllowBatchRequests extends true ? [MaybeArray<LogRecord>] : [LogRecord]
>;
type HttpHandlerGotOptionsGenerator<AllowBatchRequests extends boolean> = Created<
  AxiosRequestConfig,
  AllowBatchRequests extends true ? [MaybeArray<LogRecord>] : [LogRecord]
>;
 ...
 */
export class HttpHandler<AllowBatchRequests extends boolean> extends ProcessableHandlerMixin(
  AbstractLevelBubbleHandler,
) {
  ...
  public async handle(record: LogRecord) {
    ...
    // TODO: update requestOptions
    const config = this.url(record);

    await axios.request({
      // TypeScript does not recognize the records as correct arguments
      // @ts-ignore
      ...config,
      data: this.requestOptions(record),
    });
  ...
  public async handleBatch(records: LogRecord[]) {
    if (this.allowBatchRequests) {
      ...
      // @ts-ignore
      // TypeScript does not recognize the records as correct arguments
      await axios.request(this.url(filteredRecords), this.requestOptions(filteredRecords));
